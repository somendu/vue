...
public class P360Parts implements Parts {
  ...
  private static final List<P360Field<Part, ?>> NPA_ARTICLE_FIELDS = Arrays.asList(
          ...
          new P360Field<>("ArticleLang.DescriptionShort(EN)", P360Field.Type.STRING, Part::getPartName, Part::setPartName),
          new P360Field<>("ArticleDesign.TMEKnownInNPA", P360Field.Type.BIT, Part::getKnownInNPA, Part::setKnownInNPA));
  ...
  public Map<String, Part> get(Collection<String> identifiers) {
    ...
    // The Rest Api call byIdentifier doesn't take multiple identifiers in one call, so we need to iterate through the list
  ...
  public Map<String, Part> get(String identifier) {
    ...
    GetResponse res = rest.getForObject(
            pimProperties.getServer()
                    + "/rest/V2.0/list/Article/byIdentifiers"
    ...
    for (Row row : res.getRows()) {
      ...
      result.put(id, new Part(id,
              CastUtils.asString(row.getValues().get(1)),
              getLabel(row.getValues().get(2)),
              getLabel(row.getValues().get(3)),
  ...
  public List<Part> search(PartsFilter filter) {
    ...
    GetResponse res = rest.getForObject(
            pimProperties.getServer()
                    + "/rest/V2.0/list/Article/bySearch"
                    + "?query={query}"
                    + "&fields=" + READ_FIELDS
                    + "&pageSize=100"
                    + "&orderBy=0-ASC"
  ...
  private String generateQuery(PartsFilter filter) {
    ...
    if (!filter.getPartNumber().isEmpty()) {
      andList.add(String.format("Article.SupplierAID containsIC \"%s\"",
              getEscapedString(filter.getPartNumber())));
    }
  ...
  @Override
  /**
   * NOTE: A delete does not trigger an HTTP error status, even though the user is not allowed to do so.
   * Therefore a workaround is to compare the amount of results before and after the results.
   * When better solution arises, please implement or share :)
   */
  public void delete(String variantId, String partId, String partType) throws CouldNotSaveException {
    String query = String.format("Variant.Id+=+%s&qualificationFilter=referenceType(\"%s\"),referencedSupplierAid(\"%s\")",
            variantId, partType, partId);

    GetResponse initialPartReferences = rest.getForObject(pimProperties.getServer() +
            "/rest/V1.0/list/Variant/Variant2ArticleReference/bySearch/" +
            "?query=" + query, GetResponse.class);

    int initialRows = initialPartReferences.getRowCount();

    rest.delete(pimProperties.getServer() +
            "/rest/V1.0/list/Variant/Variant2ArticleReference/bySearch/" +
            "?query=" + query);
  ...
  /**
   * Colour and Material are ENTITY_ITEMS, therefore not as straightforward to filter on these bySearch
   * as strings. As these fields are filtered after partnumber/commodity/PIC is set when can
   * easily filter the way below.
   * @param res
   * @param searchable
   * @return
   */
